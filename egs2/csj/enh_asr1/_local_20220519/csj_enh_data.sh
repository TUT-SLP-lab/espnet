...
Usage: $0 [--stage <stage>] [--stop_stage <stop_stage>] [--nj <nj>]
  ...
  required argument:
    --background-path  : path to noise directory 
...
if [ ! -e "${CSJDATATOP}" ]; then
    log "Fill the value of 'CSJDATATOP' in db.sh"
    exit 1
fi
if [ -z "${CSJVER}" ]; then
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    # Prepare simulation data for 6ch track ＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃転写したまんま！！！！！
    ...
    python3 local/CSJ_simulate_data_patched_parallel.py --nj ${nj} --csj_path ${CSJDATATOP} --noise_path ${background_path} > local/datasimulation.log

    # Validate data simulation
    num_wavs=$(find "$odir/audio/16kHz/isolated" -iname "*.wav" | wc -l)
    if [ "$num_wavs" != "3400" ]; then # monoral 3400 + stereo 18
        log "Error: Expected 3400 wav files in '$odir/audio/16kHz/isolated', but got $num_wavs"
        exit 1
    fi
    num_wavs=$(find "$odir/audio/16kHz/isolated_ext" -iname "*.wav" | wc -l)
    if [ "$num_wavs" != "6800" ]; then
        log "Error: Expected 6800 wav files in '$odir/audio/16kHz/isolated_ext', but got $num_wavs"
        exit 1
    fi
fi
